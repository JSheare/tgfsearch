...
def translate_flags(data: pd.DataFrame, start: int | None = None, end: int | None = None,
                    only_flags: List[str] | None =None) -> pd.DataFrame:
...
def read_files(file_file: str) -> pd.DataFrame:
...
def read_file(file_name: str, passtime: Dict[str, Any] | None = None,
              killcr: bool = False) -> Tuple[pd.DataFrame, Dict[str, Any]]:
...
def _ucsc_timestring_to_datetime(time_string: str) -> datetime:
    """Returns a UCSC data file timestring as a datetime obejct."""
    parts = time_string.split()
    parts[-1] = parts[-1].rjust(6, '0')
    return datetime.strptime(' '.join(parts), '%Y %m %d %H %M %S %f')


def _custom_warning_format(message: str, category: Any, filename: Any, lineno: Any, line: Any = None) -> str:
    """Returns a warning string for the given warning info."""
    return f'Warning: {message}\n'


def _raise_warning(message: str) -> None:
    """A helper function that raises a warning with the reader's custom warning format."""
...
def _godot_lm_parser(passtime: Dict[str, Any], lines: List[str]) -> Generator[
    tuple[datetime, pd.DataFrame, float, float], None, None]:
    """A generator function that parses Godot-style list mode data files and returns a generator that yields
...
def _ssv_nrl_lm_parser(passtime: Dict[str, Any], lines: List[str]) -> Generator[
    tuple[datetime, pd.DataFrame, float, float], None, None]:
    """A generator function that parses ssv-nrl-style list mode data files and returns a generator that yields
    each frame in the file.
...
def _thor_lm_parser(passtime: Dict[str, Any], lines: List[str]) -> Generator[
    tuple[datetime, pd.DataFrame, float, float], None, None]:
    """A generator function that parses Thor-style list mode data files and returns a generator that yields
    each frame in the file.
...
def _json_nrl_lm_parser(passtime: Dict[str, Any], lines: List[str]) -> Generator[
    tuple[datetime, pd.DataFrame, float, float], None, None]:
    """A generator function that parses json-nrl-style list mode data files and returns a generator that yields
    each frame in the file.
...
def _read_lm_file(passtime: Dict[str, Any], parser: Generator[tuple[datetime, pd.DataFrame, float, float], Any, None],
                  killcr: bool) -> pd.DataFrame:
...
def _process_data_timing(passtime: Dict[str, Any], header: datetime, data: pd.DataFrame, adc_rate: float,
                         rollover_period: float) -> None:
...
def _read_trace_file(lines: List[str]) -> pd.DataFrame:
